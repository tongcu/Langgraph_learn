...
from nodes.report_generation_graph import report_generation_subgraph
# 导入你独立出来的功能
...
def custom_router(state: State):
    ...
    # 检查是否需要使用写作子图
    if state.get("next_step") == "writing_subgraph":
    ...
    if has_tool_calls:
        ...
        for call in tool_calls:
            ...
            print(f" 传入原始参数: \n {call.get('args', call.get('function', {}).get('arguments'))}")
        return "tools"
    
    # 逻辑 2: 如果没有工具调用，去 "call_model_vanilla"
    return "call_model_vanilla"
...
def start_router(state: State):
    ...
    # 检查是否需要使用报告生成子图
    if state.get("next_step") == "report_generation_subgraph":
        return "report_generation_subgraph"
    # 检查是否需要使用写作子图
    elif state.get("next_step") == "writing_subgraph" or content == "写作":
        return "writing_subgraph"
    elif summary_text == None:
...
async def call_report_generation_subgraph(state: State):
    # 调用报告生成子图
    print("Debug: 进入报告生成子图")
    subgraph_output = await report_generation_subgraph.ainvoke(state)
    print(f"报告生成子图输出: {subgraph_output}")
    # 返回子图的实际输出以更新父图状态
    return subgraph_output
...
workflow.add_conditional_edges(START, 
    start_router, 
    {
        ...
        "report_generation_subgraph":"report_generation_subgraph"
    }
)
...
workflow.add_node("report_generation_subgraph", call_report_generation_subgraph)
...
report_generation_subgraph_filename="report_generation_subgraph.png"
export_agent_graph(report_generation_subgraph, report_generation_subgraph_filename)
